...
class MainWindow(QMainWindow):
	...
	def delete_action(self):
		...
		cursor = connection.cursor()
		cursor.execute("DELETE FROM students WHERE id = ?", (main_win.table.item(selected_index, 0).text(), ))
		connection.commit()
		cursor.close()
		main_win.load_data()
...
class SearchDialog(QDialog):
	...
	def search_student(self):
		...
		cursor = connection.cursor()
		result = cursor.execute(f"SELECT * FROM students WHERE name='{self.student_name.text()}'")
		print(list(result))
